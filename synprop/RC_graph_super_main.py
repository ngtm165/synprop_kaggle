...
def save_data_to_npz(node_attrs, edge_indices, edge_attrs, ys, output_path):
    print(f"\nChuẩn bị lưu dữ liệu vào: {output_path}")

    # Chuyển đổi list của các numpy array thành một numpy array lớn với dtype=object
    # Điều này cần thiết khi các array bên trong có shape khác nhau.
    node_attrs_to_save = np.empty(len(node_attrs), dtype=object)
    for i, arr in enumerate(node_attrs):
        node_attrs_to_save[i] = arr

    edge_indices_to_save = np.empty(len(edge_indices), dtype=object)
    for i, arr in enumerate(edge_indices):
        edge_indices_to_save[i] = arr

    edge_attrs_to_save = np.empty(len(edge_attrs), dtype=object)
    for i, arr in enumerate(edge_attrs):
        edge_attrs_to_save[i] = arr
    
    # ys có thể được chuyển đổi trực tiếp vì chúng là các giá trị vô hướng
    ys_to_save = np.array(ys)
    ...
    print(f"✅ Đã lưu thành công dữ liệu vào '{output_path}'.")
...
def main():
    ...
    for split in folder_list:
        ...
        # Khởi tạo các danh sách để lưu trữ dữ liệu cho từng loại đồ thị
        normal_data, rc_data, super_data = {'na':[], 'ei':[], 'ea':[], 'y':[]}, \
                                           {'na':[], 'ei':[], 'ea':[], 'y':[]}, \
                                           {'na':[], 'ei':[], 'ea':[], 'y':[]}

        print(f"Đang trích xuất dữ liệu từ {len(graphdata)} mẫu trong dataset...")
        for i in range(len(graphdata)):
            try:
                # Lấy cả 3 loại dữ liệu từ một mẫu
                data_normal, data_rc, data_super = graphdata[i]

                # --- Thu thập dữ liệu cho đồ thị BÌNH THƯỜNG ---
                normal_data['na'].append(data_normal.x.numpy())
                normal_data['ei'].append(data_normal.edge_index.numpy())
                normal_data['ea'].append(data_normal.edge_attr.numpy())
                normal_data['y'].append(data_normal.y.item()) # .item() để lấy giá trị vô hướng

                # --- Thu thập dữ liệu cho đồ thị RC ---
                # Kiểm tra nếu đồ thị RC không rỗng
                if data_rc.x.shape[0] > 0:
                    rc_data['na'].append(data_rc.x.numpy())
                    rc_data['ei'].append(data_rc.edge_index.numpy())
                    rc_data['ea'].append(data_rc.edge_attr.numpy())
                    rc_data['y'].append(data_rc.y.item())

                # --- Thu thập dữ liệu cho đồ thị SUPERNODE ---
                # Kiểm tra nếu đồ thị Supernode không rỗng
                if data_super.x.shape[0] > 0:
                    super_data['na'].append(data_super.x.numpy())
                    super_data['ei'].append(data_super.edge_index.numpy())
                    super_data['ea'].append(data_super.edge_attr.numpy())
                    super_data['y'].append(data_super.y.item())
                
                if (i + 1) % 50 == 0: # In tiến trình
        ...
        output_folder = './output/RC/directed/barriers_cycloadd'
