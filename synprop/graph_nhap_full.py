...
class ReactionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for idx, bond in enumerate(lst_edges_update): # bond là cặp (chỉ số nút u, chỉ số nút v)
            ...
            edge_fea_RC = edge_fea3 + edge_fea2 if order_0 > order_1 else edge_fea3 + edge_fea1

            if any(x != 0 for x in changes):
                bond_info = {
                    'edge_indices': {(u,v),(v,u)},
                    'edge_features': edge_fea_RC,
                    'node_features': {
                        u: atom_fea_graph[u],
                        v: atom_fea_graph[v]
                    }
        ...
        for bond_data in reindexed_rc_data:
            ...
            edge_feat_graph.append(directed_feature_rc_uv)
            directed_feature_rc_vu = atom_feat_rc_v + RC_edge_features
            edge_feat_graph.append(directed_feature_rc_vu)
        ...
        row += RC_row
        col += RC_col
        # print(f"features egde: {edge_feat_graph}")
...
def main():
    ...
    data_path=f'./Data/regression/USPTO_50k/USPTO_50k_aam.csv'
    graph_path=f'./Data/regression/USPTO_50k/its_new/USPTO_50k_aam.pkl.gz'
    target='new_class'
    ...
    all_node_attrs = []
    all_ys = []
    output_filename = f'USPTO_50k_aam_processed_data.npz'
