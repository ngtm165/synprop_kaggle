...
def read_data(data_path, graph_path, target):
    graph, labels = [], []
    with gzip.open(data_path, 'rt', newline='') as csvfile:
...
def main():
    repeats = [f'repeat_{i}' for i in range(5)] 
    ...
    for re in repeats: 
        for fold in folds:
            for name in name_file: 
                data_path=f'./Data/Data_final/rdb7/{re}/{fold}/rdb7_{name}_{re}_{fold}.csv.gz'
                graph_path=f'./Data/Data_final/its_2025/rdb7/{re}/{fold}/rdb7_aam_{name}.pkl.gz'
                target='ea'
                graphdata=ReactionDataset(data_path,graph_path,target)
                print(graphdata.__getitem__(8))

                # --- Thiết lập thư mục và file output ---
                output_folder = f'output/final/rdb7/{re}/{fold}'                                  ## <-- THÊM MỚI: Định nghĩa tên thư mục output
                ...
                output_filename = f'rdb7_aam_{name}_processed_data.npz'
