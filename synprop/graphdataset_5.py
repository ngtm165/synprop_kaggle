...
def main():
    ...
    # name_file = ['val', 'test', 'train']
    # for fold in folder_list:
    #     for name in name_file: 
    #         data_path=f'./Data/regression/rad6re/{fold}/aam_{name}.csv'
    #         graph_path=f'./Data/regression/rad6re/its_new/{fold}/rad6re_aam_{name}.pkl.gz'
    #         target='dh'
    #         graphdata=ReactionDataset(data_path,graph_path,target)
    #         print(graphdata.__getitem__(8))

    #         # --- Thiết lập thư mục và file output ---
    #         output_folder = f'output/rad6re/{fold}'                                  ## <-- THÊM MỚI: Định nghĩa tên thư mục output
    ...
    #         output_filename = f'rad6re_aam_{name}_processed_data.npz'
    ...
    #                 print(f"Lỗi khi xử lý mẫu {fold}: {e}")
    ...
    for i in folder_list:
        data_path=f'./Data/regression/barriers_sn2/{i}.csv'
        graph_path=f'./Data/regression/barriers_sn2/its_new/barriers_sn2_aam_{i}.pkl.gz'
        target='ea'
        graphdata=ReactionDataset(data_path,graph_path,target)
        print(graphdata.__getitem__(8))
        ...
        all_node_attrs = []
        all_ys = []
        output_filename = f'barriers_sn2_aam_{i}_processed_data.npz'
        
            # Tạo đường dẫn file output hoàn chỉnh
        
        output_folder = './output/barriers_sn2'
