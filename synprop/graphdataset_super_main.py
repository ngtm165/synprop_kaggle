...
def main():
    ...
    for i in folder_list:
        data_path=f'./Data/regression/e2sn2/{i}/aam_train.csv'
        graph_path=f'./Data/regression/e2sn2/its_new/{i}/e2sn2_aam_train.pkl.gz'
        target='act'
        graphdata=ReactionDataset(data_path,graph_path,target)
        print(graphdata.__getitem__(8))

        # --- Thiết lập thư mục và file output ---
        output_folder = f'output/e2sn2/{i}'                                  ## <-- THÊM MỚI: Định nghĩa tên thư mục output
        ...
        output_filename = 'e2sn2_aam_train_processed_data.npz'
