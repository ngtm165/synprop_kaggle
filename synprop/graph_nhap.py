...
def main():
    ...
    base_data_path = './Data/regression/USPTO_50k'
    output_folder = './output/RC/full/USPTO_50k'
    os.makedirs(output_folder, exist_ok=True) 

    print(f"\n{'='*20} BẮT ĐẦU XỬ LÝ: {'USPTO_50'.upper()} {'='*20}")
    data_path = os.path.join(base_data_path, f'USPTO_50k_aam.csv')
    graph_path = os.path.join(base_data_path, 'its_new', f'USPTO_50k_aam.pkl.gz')
    target = 'new_class'
    ...
    print(f"Đang trích xuất dữ liệu từ {len(graphdata)} mẫu trong 'USPTO_50'...")
    for i in range(len(graphdata)):
        try:
            ...
            print(f"❌ Lỗi khi xử lý mẫu {i} trong tập USPTO_50: {e}")
    ...
    for name, data_dict in all_data.items():
        if not data_dict['node_attrs']:
            print(f"\n⚠️ Không có dữ liệu để lưu cho loại '{name}' trong tập 'USPTO_50'. Bỏ qua.")
            continue
            
        output_filename = f'USPTO_50k_aam_{name}_processed_data.npz'
        output_npz_file = os.path.join(output_folder, output_filename)
        save_to_npz(data_dict, output_npz_file)
        
    print(f"\n{'='*20} HOÀN TẤT XỬ LÝ: {'USPTO_50'.upper()} {'='*20}")
